...
void primeFactorsD(struct PrimeFactorization * pf){
    ...
    unsigned count = 0;
    struct PrimeFactor* decomposition = primeFactorsC(&count, 84);
    pf->number = 84;
    pf->count = count;
    pf->primeFactors = decomposition;
    printf("%d, %d, ", pf->number, pf->count);
    for (unsigned int i = 0; i < pf->count; i++){
        printf("{%d,%d} ", pf->primeFactors[i].value, pf->primeFactors[i].multiplicity);
    }
    printf("\n");
