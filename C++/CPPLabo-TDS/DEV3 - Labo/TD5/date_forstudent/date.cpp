...
Date::Date(int year, unsigned mounth, unsigned day) :

    year_ { validate(year, Date::MINIMUM_YEAR, Date::MAXIMUM_YEAR) },
    month_ { validate(mounth, Date::MINIMUM_MONTH, Date::MAXIMUM_MONTH) },
    day_ { validate(day, Date::MINIMUM_DAY, Date::MAXIMUM_DAY[mounth]) }
{ }

std::string Date::to_string() const
{
    return std::to_string(Date::Date);
}
