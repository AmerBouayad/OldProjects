...
void print72 (char name[72]){ // name est un tableau de 72 caratctère
	printf ("name : ");
	for (int i=0; i< 72; i++)
		printf ("%c", name[i]);
	printf ("\n");
...
int main(int argc, char * argv[]){
	...
	// lecture de LBA1 : entète primaire GPT
	...
	//ROUE LIBRE
	lseek(fd,-(33*512)+sizeof(entryGpt_t),SEEK_END); // revenir aux descripteurs : entry 2
	read(fd, &entryGPT,sizeof(entryGpt_t));
	lseek (fd,512+sizeof(headerGpt_t),SEEK_SET);
	write(fd, &entryGPT, sizeof(entryGpt_t));
